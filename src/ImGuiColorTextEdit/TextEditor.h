...
public:
	...
	// ── Geriye dönük uyumluluk shim'leri ─────────────────────────────────────
	inline bool IsTextChanged() const { return mTextChanged; }
	inline bool IsOverwrite()   const { return false; }
	inline bool IsShowingWhitespaces() const { return IsShowWhitespacesEnabled(); }
	inline void SetShowWhitespaces(bool v)   { SetShowWhitespacesEnabled(v); }
	inline int  GetTotalLines() const        { return GetLineCount(); }
	inline bool HasSelection()  const        { return AnyCursorHasSelection(); }
	inline void InsertText(const std::string& s) { InsertTextAtCursor(s.c_str()); }
	inline void InsertText(const char* s)        { InsertTextAtCursor(s); }
	// GetCursorPosition() → Coordinates (eski API; imzası new fork'un void overload'ı ile çakışmaz)
	inline Coordinates GetCursorPosition() const { return GetSanitizedCursorCoordinates(); }
	// SetPalette / SetLanguageDefinition struct tabanlı overload'lar
	void SetPalette(const Palette& aValue);
	void SetLanguageDefinition(const LanguageDefinition& aLangDef);
