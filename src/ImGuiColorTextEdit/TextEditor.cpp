...
void TextEditor::SetLanguageDefinition(LanguageDefinitionId aValue)
{
	...
	case LanguageDefinitionId::GCode:
		...
		break;
	case LanguageDefinitionId::Rust:
		mLanguageDefinition = &(LanguageDefinition::Rust());
		break;
	}
...
// ── Compat overload: Palette dizisini doğrudan ata (alpha karışımı App katmanında yapılmış sayılır)
void TextEditor::SetPalette(const Palette& aValue)
{
	mPalette = aValue;
}

// ── Compat overload: Struct tabanlı LanguageDefinition (EditorTab::LangByIdx kalıntısı)
void TextEditor::SetLanguageDefinition(const LanguageDefinition& aLangDef)
{
	mLanguageDefinitionId = LanguageDefinitionId::None;
	mLanguageDefinition   = &aLangDef; // caller'ın statik ömürlü nesne vermesi beklenir
	mRegexList->mValue.clear();
	for (const auto& r : mLanguageDefinition->mTokenRegexStrings)
...
{
	mTextChanged = false;   // Her frame başında sıfırla; AddUndo yeniden set eder
...
void TextEditor::HandleKeyboardInputs(bool aParentIsFocused)
{
	if (ImGui::IsWindowFocused() || aParentIsFocused)
	{
		...
		// Shift+Alt+↑/↓ → yeni imleç ekle (çoklu imleç)
		else if (alt && shift && !ctrl && !super && ImGui::IsKeyPressed(ImGuiKey_UpArrow))
		{
			Coordinates newPos = mState.mCursors[mState.mCurrentCursor].mInteractiveEnd;
			MoveCoords(newPos, MoveDirection::Up, false, 1);
			mState.AddCursor();
			SetCursorPosition(newPos, mState.mCurrentCursor, true);
			MergeCursorsIfPossible();
		}
		else if (alt && shift && !ctrl && !super && ImGui::IsKeyPressed(ImGuiKey_DownArrow))
		{
			Coordinates newPos = mState.mCursors[mState.mCurrentCursor].mInteractiveEnd;
			MoveCoords(newPos, MoveDirection::Down, false, 1);
			mState.AddCursor();
			SetCursorPosition(newPos, mState.mCurrentCursor, true);
			MergeCursorsIfPossible();
		}
