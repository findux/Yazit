...
const TextEditor::Palette & TextEditor::GetDarkPalette()
{
	const static Palette p = { {
			0xffe0e0e0,	// Default  (plain-text için yüksek kontrast: açık gri → koyu zemin)
			0xffd69c56,	// Keyword
			0xff00ff00,	// Number
			0xff7070e0,	// String
			...
			0xff4db34d, // Comment (single line) — daha okunabilir yeşil
			0xff6b8c3e, // Comment (multi line)  — daha okunabilir yeşil
...
{
	const static Palette p = { {
			0xff1a1a1a,	// None     (plain-text için yüksek kontrast: koyu → beyaz zemin)
			0xffff0c06,	// Keyword
			0xff008000,	// Number
			0xff2020a0,	// String
			...
			0xff1e1e1e, // Identifier — daha koyu, daha okunabilir
			0xff4d6e00, // Known identifier
			0xffc040a0, // Preproc identifier
			0xff1a5c1a, // Comment (single line) — koyu arka plan için daha derin yeşil
			0xff2e4a10, // Comment (multi line)
...
void TextEditor::EnsureCursorVisible()
{
	...
	float charW      = mCharAdvance.x;
	float scrollbarH = ImGui::GetStyle().ScrollbarSize;
	float visH       = height - scrollbarH;   // yatay scrollbar her zaman göründüğü için
	float visW       = width;

	float cy0 = (float)pos.mLine * lineH;     // imlecin üst piksel sınırı
	float cy1 = cy0 + lineH;                  // imlecin alt piksel sınırı
	float cx  = len + mTextStart;

	// Dikey: SADECE imleç görünür alanın dışına taştığında kaydır
	if (cy0 < scrollY)
		ImGui::SetScrollY(cy0);
	else if (cy1 > scrollY + visH)
		ImGui::SetScrollY(cy1 - visH);

	// Yatay: SADECE imleç görünür alanın dışına taştığında kaydır
	if (cx < scrollX)
		ImGui::SetScrollX(cx);
	else if (cx + charW > scrollX + visW - charW)
		ImGui::SetScrollX(cx + charW - visW + charW);
}
