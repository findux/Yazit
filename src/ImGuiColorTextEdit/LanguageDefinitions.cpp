...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GCode()
{
	...
	if (!inited)
	{
		...
		// N satır numaraları: N10, N0100
		langDef.mTokenRegexStrings.push_back({ R"([Nn][0-9]+)", PI::GCodeLineNum });
		// Hareket G-kodları: G0-G4, G28, G29, G30, G38, G73, G76, G80-G89
		langDef.mTokenRegexStrings.push_back({ R"([Gg](0*[0-4]|2[89]|3[08]|7[36]|8[0-9])(?![0-9]))", PI::GCodeMotion });
		// Kalan modal G-kodları
		langDef.mTokenRegexStrings.push_back({ R"([Gg][0-9]+)", PI::GCodeModal });
		// M-kodları
		langDef.mTokenRegexStrings.push_back({ R"([Mm][0-9]+)", PI::GCodeMCode });
		// Eksen harfleri + değer: X Y Z A B C
		langDef.mTokenRegexStrings.push_back({ R"([XYZABCxyzabc][+-]?([0-9]+\.?[0-9]*|\.[0-9]+))", PI::GCodeAxis });
		// Besleme/hız/takım: F S T H D
		langDef.mTokenRegexStrings.push_back({ R"([FSTHDfsthdDd][+-]?([0-9]+\.?[0-9]*|\.[0-9]+))", PI::GCodeFeed });
		// Yay/döngü parametreleri: I J K R P Q L E
		langDef.mTokenRegexStrings.push_back({ R"([IJKRPQLEijkrpqle][+-]?([0-9]+\.?[0-9]*|\.[0-9]+))", PI::GCodeArc });
		// Serbest sayılar
		langDef.mTokenRegexStrings.push_back({ R"([+-]?([0-9]+\.?[0-9]*|\.[0-9]+))", PI::Number });
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Rust()
{
	...
	if (!inited)
	{
		...
		static const char* const builtins[] = {
			...
			"usize","f32","f64","char","str","String","Vec","Option","Result","Some","None",
			"Ok","Err","Box","Rc","Arc","Cell","RefCell","HashMap","HashSet","println",
			"print","eprintln","eprint","panic","assert","assert_eq","assert_ne","todo",
			"unimplemented","unreachable","format","write","writeln"
		};
		for (auto b : builtins) {
			Identifier id;
			id.mDeclaration = "Rust yerleşik";
			langDef.mIdentifiers.insert({ b, id });
		}

		langDef.mTokenRegexStrings.push_back({ R"(//.*)", PaletteIndex::Comment });
		langDef.mTokenRegexStrings.push_back({ R"(\"(?:\.|[^\"])*\")", PaletteIndex::String });
		langDef.mTokenRegexStrings.push_back({ R"('(?:\.|[^'])')", PaletteIndex::CharLiteral });
		langDef.mTokenRegexStrings.push_back({ R"(0x[0-9a-fA-F]+|[0-9]+\.?[0-9]*(?:[eE][+-]?[0-9]+)?(?:_[a-z0-9]+)?)", PaletteIndex::Number });
